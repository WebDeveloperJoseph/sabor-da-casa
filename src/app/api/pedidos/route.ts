...
const itemPedidoSchema = z.object({
  ...
  adicionais: z
    ...
    .optional(),
  bordaId: z.number().int().optional(),
  bordaNome: z.string().optional(),
  bordaPreco: z.number().min(0).optional(),
  ...
  try {
    ...
    if (itensNormais.length > 0) {
      ...
      if (pratos.length !== pratoIdsUnicos.length) {
        ...
        const pratosNaoEncontrados = pratoIdsUnicos.filter(
          (id) => !pratosEncontrados.includes(id),
        );

        return NextResponse.json(
          {
            erro: "Alguns pratos não foram encontrados ou estão inativos",
            pratosNaoEncontrados,
          },
          { status: 400 },
        );
      }
    ...
    console.log(
      `Pedido com ${itensNormais.length} itens normais e ${itensMistos.length} pizzas mistas`,
    );

    // Calcular valor total considerando tamanhos e extras (adicionais/borda)
    let valorTotal = 0;
    const itensParaCriar = itens.map((item) => {
      const adicionaisTotal = (item.adicionais || []).reduce(
        (acc, adicional) => {
          return acc + Number(adicional.preco || 0);
        },
        0,
      );
      const bordaExtra = Number(item.bordaPreco || 0);
      ...
      if (item.pratoId === 999) {
        ...
        // Mantém o preço enviado para pizza mista, pois ele já é calculado no cliente.
      ...
      if (item.tamanho && prato.tamanhos.length > 0) {
        const tamanhoEncontrado = prato.tamanhos.find(
          (t: any) => t.tamanho === item.tamanho,
        );
        if (tamanhoEncontrado) {
          precoUnit = Number(tamanhoEncontrado.preco);
        } else {
          throw new Error(
            `Tamanho ${item.tamanho} não disponível para ${prato.nome}`,
          );
        }
      }
      const precoUnitFinal = precoUnit + adicionaisTotal + bordaExtra;
      const subtotal = precoUnitFinal * item.quantidade;
      valorTotal += subtotal;
      return {
        pratoId: item.pratoId,
    ...
    try {
      pedido = await prisma.$transaction(async (tx) => {
        ...
        const counter = (
          counterRows as unknown as Array<{ last?: number }>
        )?.[0];
    ...
    const responseBody = {
      id: pedido.id,
      dailyNumber:
        (pedido as unknown as { dailyNumber?: number }).dailyNumber ?? null,
    };
