  ...
  try {
    ...
    await prisma.prato.update({
      ...
      data: {
        ...
        categoriaId:
          categoriaId !== undefined ? Number(categoriaId) : exist.categoriaId,
    ...
    if (Array.isArray(ingredientes)) {
      await prisma.$transaction([
        ...
        ...ingredientes.map((ing: number) =>
          prisma.pratoIngrediente.create({
            data: { pratoId: idNum, ingredienteId: Number(ing) },
          }),
        ),
      ]);
    }
        ...
        ...tamanhos.map((t: { tamanho: string; preco: number }) =>
          prisma.pratoTamanho.create({
            data: {
              ...
              preco:
                typeof t.preco === "string" ? parseFloat(t.preco) : t.preco,
            },
          }),
        ),
      ]);
...
export async function DELETE(
  ...
  try {
    ...
    const itensRelacionados = await prisma.itemPedido.count({
      where: { pratoId: idNum },
    });

    // Se o prato já participou de pedidos, preservamos histórico e apenas desativamos.
