...
export default function FormularioPrato({
  ...
  const [preco, setPreco] = useState<string>(
    initial ? String(initial.preco) : "",
  );
  const [imagem, setImagem] = useState(initial?.imagem ?? "");
  const [categoriaId, setCategoriaId] = useState<string>(
    initial ? String(initial.categoriaId) : "",
  );
  ...
  const [ingredientesSelecionados, setIngredientesSelecionados] = useState<
    number[]
  >(initial?.ingredientesIds ?? []);
  const [salvando, setSalvando] = useState(false);
  const [imagemUploading, setImagemUploading] = useState(false);

  // Estados para tamanhos (P, M, G)
  const [usarTamanhos, setUsarTamanhos] = useState<boolean>(
    !!(initial?.tamanhos && initial.tamanhos.length > 0),
  );
  const [precoP, setPrecoP] = useState<string>(
    initial?.tamanhos?.find((t) => t.tamanho === "P")?.preco.toString() ?? "",
  );
  const [precoM, setPrecoM] = useState<string>(
    initial?.tamanhos?.find((t) => t.tamanho === "M")?.preco.toString() ?? "",
  );
  const [precoG, setPrecoG] = useState<string>(
    initial?.tamanhos?.find((t) => t.tamanho === "G")?.preco.toString() ?? "",
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const url =
        modo === "create" ? "/api/pratos" : `/api/pratos/${initial?.id}`;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Marque para cadastrar preços por tamanho
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="text-xs text-muted-foreground">
              ou envie um arquivo para o Storage
            </div>
            <UploadImagem
              onUploaded={(url) => setImagem(url)}
              onUploadingChange={setImagemUploading}
            />
            {imagemUploading && (
              <p className="text-xs text-amber-700">
                Upload em andamento. O botão salvar será liberado ao concluir.
              </p>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-3">
        <Button
          type="submit"
          disabled={!isValid || salvando || imagemUploading}
        >
          {salvando
            ? "Salvando..."
            : modo === "create"
