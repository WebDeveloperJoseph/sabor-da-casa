...
const enableAdminRealtime =
  process.env.NEXT_PUBLIC_ENABLE_ADMIN_REALTIME === "true";

export default function AdminOrderRealtimeNotifier() {
  const [usePolling, setUsePolling] = useState(
    () => !supabase || !enableAdminRealtime,
  );
  const eventReceivedRef = useRef(false);

  useEffect(() => {
    if (!enableAdminRealtime) {
      return;
    }
      ...
      .on(
        ...
        (payload) => {
          try {
            ...
            console.debug("AdminOrderRealtimeNotifier: novo evento", {
              id,
              nome,
              payload,
            });
    ...
    const fallbackTimeout = setTimeout(() => {
      if (!eventReceivedRef.current) {
        console.warn(
          "AdminOrderRealtimeNotifier: nenhum evento realtime recebido — ativando polling fallback",
        );
        setUsePolling(true);
      }
    }, 20000);
    ...
    return () => {
      try {
        ...
        console.warn("Erro ao unsubscribir canal", e);
      }
